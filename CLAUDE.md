...
Its check is SQL, and **it must be `>= 40`, not `> 40`** — measured and
corrected 2026-09-09:

```sql
-- Any row is a runaway loop the ceiling STOPPED. Not "this should return
-- nothing" — that was the pre-ceiling form of this check and it is wrong now.
SELECT call_sid, agent_used, tool_call_count FROM call_logs
WHERE voice_provider = 'grok' AND tool_call_count >= 40;
```

`toolCeiling.ts` refuses at `this.dispatches >= this.limits.perCallDispatches`
with `perCallDispatches: 40`, so **a call can reach 40 and can never exceed
it.** The published `> 40` form could only ever match calls from before the
ceiling shipped. It proved the deploy and then went blind, while remaining the
only thing watching for loops.

Measured 2026-09-09, all grok rows:

| | |
|---|---|
| `tool_call_count > 40` — what the old check looked for | **1** (the pre-ceiling optical call, 118, 2026-09-03) |
| `= 40` — loops the ceiling stopped, all missed by the old check | **5** |
| between 25 and 39 | **0** |

**The zero between 25 and 39 is what makes this unambiguous:** 40 is not a
value calls drift to, it is the ceiling being hit. The five are
`CA3985d8bcabd63bb29e7861a58cfc682b` (optical, 09-04),
`CAefbdd1832725217d5846f723c259f944` (optical, 09-04),
`CA60675f75fcbb9211e68dc01e7416a83f` (pcp, 09-04),
`CA9f9710a9fe054f387f1d6e4c6f3b6350` (optical, 09-08) and
`CA3ccec8b38c1734b990f7f6c91fec71e6` (optical, 09-08). What each loop actually
was has NOT been established — do not assume `resolve_location`.

The same wrong form is in `docs/PULL-CHECK.md`; both were fixed together.
