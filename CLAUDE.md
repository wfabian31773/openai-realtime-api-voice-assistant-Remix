...
NEWEST, v62 IS WHAT `main` CARRIES, AND v57 AND v60 ARE BOTH RETIRED RATHER
THAN REUSED — see THE SURGEON CLAIM below the table for v57.** **NEWEST IS NOT
THE SAME AS CONTAINED, and on this row they do NOT agree:** v65 branched off
the v62 `main`, so it contains v62 and contains NEITHER **v63** (#326, the
callback's provenance) NOR **v64** (#327, the caller's audio) — those two are
SIBLINGS off the same base, and all three numbers were assigned up front
precisely so three open branches could never make three builds read alike at
`/voice/health`. Whichever lands last merges `main` and re-bumps above it.
**v59 IS STILL CLAIMED BY AN OPEN BRANCH**, #293 (RULE ZERO 2a's gate), which
branched off v10 and contains none of this; it is a SIBLING below, it must
merge `main` and re-bump above whatever `main` then carries, and its number
says nothing about what it holds. **A build must never read v60:** it described
#323's change and was never on `main`, which re-bumped to v62 for exactly this
reason. The next ship after this takes **v66**. v19-v24 were siblings off v18 on 2026-09-15 — v19 the PCP lost-requestfloor (#300), v20 the blind transfer telemetry (#302), v21 the ask detection
