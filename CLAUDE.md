...
NEWEST, v62 IS WHAT `main` CARRIES SINCE #323 MERGED 2026-09-24 (`5c76d2f`),
AND v57 AND v60 ARE BOTH RETIRED RATHER THAN REUSED — see THE SURGEON CLAIM
below the table for v57.** Two corrections landed on this line in two days,
and both were the same trap: it read `v58 IS WHAT main CARRIES` after #324 had
moved `main` to v61, and `v61 IS WHAT main CARRIES` after #323 moved it to
v62. Left either way, a reader would have accepted a superseded deployment as
current — which is the one thing this table exists to prevent. **So re-read
this line against `main` rather than trusting it; it goes stale every time a
marker lands.** **NEWEST IS NOT THE SAME AS CONTAINED**, and here they agree
only because the merges made them agree: v63 contains v62 because `main` was
merged into it on 2026-09-24, not because 63 > 62. **A build must never read
v60:** it described #323's change and was never on `main`, which re-bumped to
v62 for exactly this reason. **v59 IS STILL CLAIMED BY AN OPEN BRANCH**, #293
(RULE ZERO 2a's gate), which branched off v10 and contains none of this; it is
a SIBLING BELOW `main`, it must merge `main` and re-bump above v63 before it
lands, and its number says nothing about what it holds. The next ship after
this takes **v64**.
