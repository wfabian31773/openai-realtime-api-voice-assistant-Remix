...
**AND THE SEQUENCE HAS A HOLE IN IT ON PURPOSE: v13 IS SKIPPED. v66 IS THE
...
been corrected three days running and always the same way: `v58 IS WHAT main
CARRIES` after #324 moved `main` to v61, `v61` after #323 moved it to v62, and
`v62` after #327 moved it to v64 — that last one arrived ON `main` already
...
the open numbers".** v66 contains v64 because `main` was merged into it on
2026-09-24, not because 66 > 64, and it needed no re-bump: it was already above
`main`, so `/voice/health` cannot go backwards on it. The other open siblings
are **v65** (#328, the silence ladder) and **v67** (#326, the callback
provenance — **re-bumped from v63 by that same merge**, because v63 fell BELOW
`main` and a marker below `main` reads as a failed pull). None of the three
contains another. **A build must never read v60 or v63:** v60 described #323's
change and never reached `main`, v63 described #326's and never reached it
either. **v59 IS STILL CLAIMED BY AN OPEN BRANCH**, #293 (RULE ZERO 2a's gate),
which branched off v10, contains none of this, sits far BELOW `main`, and must
merge and re-bump above whatever `main` carries at that moment. The next ship
after the three takes **v68**. v19-v24 were siblings off v18 on 2026-09-15 — v19 the PCP lost-requestfloor (#300), v20 the blind transfer telemetry (#302), v21 the ask detection
...
**v62 (#323) IS WHAT `main` CARRIES (merged 2026-09-24, `5c76d2f`), AND v57 AND
v60 ARE RETIRED RATHER THAN REUSED.** v60 was #323's own number while it sat
below `main`; the moment it merged that `main` it took **v62**, because two
...
number BELOW `main` reads as a failed pull. **This branch is v66 for exactly
that reason**, with v63, v64 and v65 the three open siblings in between.
**v59 is still claimed by the open #293** and stays reserved for it.
