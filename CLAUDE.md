...
**AND THE SEQUENCE HAS A HOLE IN IT ON PURPOSE: v13 IS SKIPPED. v67 IS THE
NEWEST, v64 IS WHAT `main` CARRIES SINCE #327 MERGED 2026-09-24 (`e678807`),
AND v57, v60 AND v63 ARE ALL RETIRED RATHER THAN REUSED — see THE SURGEON
CLAIM below the table for v57.** **THIS LINE GOES STALE EVERY TIME A MARKER
LANDS, SO RE-READ IT AGAINST `main` RATHER THAN TRUSTING IT.** Three
corrections have landed on it in three days and every one was the same trap:
it read `v58 IS WHAT main CARRIES` after #324 moved `main` to v61, `v61` after
#323 moved it to v62, and `v62` after #327 moved it to v64 — the version that
arrived ON `main` with #327 still said v62, because a branch's own prose does
not update itself when that branch merges. Left any of those ways, a reader
accepts a superseded deployment as current, which is the one thing this table
exists to prevent.

**NEWEST IS NOT THE SAME AS CONTAINED, and on this row they disagree four
ways.** v67 contains v64 because `main` was merged into it, not because
67 > 64. **v65 (#328, the silence ladder) and v66 (#325, the PCP floor and its
own re-bump) are OPEN SIBLINGS off the v62 `main`**, so they contain neither
v64 nor each other nor this; each must merge `main` and re-bump above whatever
`main` then carries before it lands. This branch being the HIGHEST of the four
means only that its number was allocated last — it held **v63** until #327
merged and put that number BELOW `main`, which reads as a failed pull.
**A build must never read v60 or v63:** v60 described #323's change and was
never on `main`, and v63 described this one and never reached it either.
**v59 IS STILL CLAIMED BY AN OPEN BRANCH**, #293 (RULE ZERO 2a's gate), which
branched off v10, contains none of this, sits far BELOW `main`, and must merge
and re-bump above whatever `main` carries at that moment; its number says
nothing about what it holds. The next ship after this takes **v68**. v19-v24 were siblings off v18 on 2026-09-15 — v19 the PCP lost-requestfloor (#300), v20 the blind transfer telemetry (#302), v21 the ask detection
