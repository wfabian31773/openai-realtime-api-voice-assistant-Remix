...
voice-runtime-v63-the-callback-says-where-it-came-from-20260924
```
...
**AND THE SEQUENCE HAS A HOLE IN IT ON PURPOSE: v13 IS SKIPPED. v63 IS THE
NEWEST, v61 IS WHAT `main` CARRIES SINCE #324 MERGED 2026-09-23 (`7de4dd7`),
AND v57 WAS WITHDRAWN — see THE SURGEON CLAIM below the table.** This line
read `v58 IS WHAT main CARRIES` until that merge, and leaving it would have
armed the exact trap this table exists to disarm: a reader would have believed
a deployment on v58 was current. **NEWEST IS NOT THE SAME AS CONTAINED**, and
v59/v60/v62 are the case that proves it: v59 (#293) and v60+v62 (#323) branched
off the v58 `main` used to carry, so each is a sibling of nothing below it and
**BOTH NOW SIT BELOW `main`** — either landing as-is would send
`/voice/health` BACKWARDS, which reads as a failed pull rather than a new
build, so both must merge `main` and re-bump above v63 before they land.
**v63 (#326) branched off the v61 on `main`, so it contains v61 and none of
v59, v60 or v62.** A higher number still says nothing about containment:
61 > 60 did not mean v61 contained v60, and it did not. v19-v24 were siblings off v18 on 2026-09-15 — v19 the PCP lost-request
...
never read v57; **v58 is that next ship and it is the CURRENT marker** — the
identity probe, an instrument and no behaviour change (see the v56 row below
for what it measures).

**v63 IS THE CURRENT MARKER. v61 (#324) IS WHAT `main` CARRIES, AND 59, 60
AND 62 ARE SKIPPED RATHER THAN TAKEN.** v59 is claimed by the open #293, v60
and v62 by the open #323, and two branches carrying one version make two
builds indistinguishable at `/voice/health` — the single thing this constant
exists to prevent. So the identity-write instrument took the next free number
rather than colliding, and the callback-provenance ship did the same.
**v63 branched off the v61 that `main` carries, so it contains v61 and NOT
v59, v60 or v62** — #293 and #323 must each merge `main` and re-bump ABOVE
v63 before they land, or `/voice/health` goes backwards and reads as a failed
pull. Containment is stated once, in the sequence sentence above; see the v56
row for what v61 measures and the v61 row for what v63 changes.
