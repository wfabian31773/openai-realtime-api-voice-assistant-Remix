...
**AND THE SEQUENCE HAS A HOLE IN IT ON PURPOSE: v13 IS SKIPPED. v65 IS THE
NEWEST HERE, v64 IS WHAT `main` CARRIES SINCE #327 MERGED 2026-09-24
(`e678807`), AND v57, v60 AND v63 ARE ALL RETIRED RATHER THAN REUSED — see THE
SURGEON CLAIM below the table for v57.** **THIS LINE GOES STALE EVERY TIME A
MARKER LANDS, SO RE-READ IT AGAINST `main` RATHER THAN TRUSTING IT.** It has
been corrected three days running and always the same way: it read `v58 IS WHAT
main CARRIES` after #324 moved `main` to v61, `v61` after #323 moved it to v62,
and `v62` after #327 moved it to v64 — that last one arrived ON `main` already
wrong, because a branch's own prose does not update itself when the branch
lands.

**NEWEST IS NOT THE SAME AS CONTAINED, and "newest" here means only "newest of
the four open numbers".** v65 contains v64 because `main` was merged into it on
2026-09-24, not because 65 > 64, and it did NOT need re-bumping: it was already
above `main`, so `/voice/health` cannot go backwards on it. The other two open
siblings are **v66** (#325, the PCP lost-request floor) and **v67** (#326, the
callback provenance — **re-bumped from v63 by that same merge**, because v63
fell BELOW `main` and a marker below `main` reads as a failed pull). Neither
contains this, and this contains neither. **A build must never read v60 or
v63:** v60 described #323's change and never reached `main`, and v63 described
#326's and never reached it either. **v59 IS STILL CLAIMED BY AN OPEN BRANCH**,
#293 (RULE ZERO 2a's gate), which branched off v10, contains none of this, sits
far BELOW `main`, and must merge and re-bump above whatever `main` carries at
that moment. The next ship after the four takes **v68**. v19-v24 were siblings off v18 on 2026-09-15 — v19 the PCP lost-requestfloor (#300), v20 the blind transfer telemetry (#302), v21 the ask detection
