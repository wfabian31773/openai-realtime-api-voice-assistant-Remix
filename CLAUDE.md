...
MARKER LANDS, SO RE-READ IT AGAINST `main` RATHER THAN TRUSTING IT.** Four
...
#323 moved it to v62, `v62` after #327 moved it to v64, and `v64` after #326
moved it to v67 — those last two arrived ON `main` already wrong, because a
branch's own prose does not update itself when that branch merges.

**NEWEST IS NOT THE SAME AS CONTAINED, and "newest" here means only "newest of
the two open numbers".** v69 contains v67 because `main` was merged into it on
2026-09-24, not because 69 > 67. This branch has now moved three times — v61
below the v58 `main`, v66 when `main` reached v62, KEPT at v66 when #327 moved
`main` to v64 because 66 > 64, and v69 now that #326 has put `main` at v67 —
which is the rule working, not churn: a marker re-bumps only when it falls
BELOW `main`, where it would read as a failed pull. **The one remaining sibling
is v68** (#328, the silence ladder, re-bumped from v65 above the same `main`).
It does not contain this and this does not contain it, and both now sit above
`main`, so neither needs anything further whichever lands first. **A build must
never read v60, v63, v65 or v66:** v60 described #323's change, v63 #326's, v65
#328's and v66 this one, and none of the four ever reached `main`. **v59 IS
STILL CLAIMED BY AN OPEN BRANCH**, #293 (RULE ZERO 2a's gate), which branched
off v10, contains none of this, sits far BELOW `main`, and must merge and
re-bump above whatever `main` carries at that moment. The next ship after the
two takes **v70**. v19-v24 were siblings off v18 on 2026-09-15 — v19 the PCP lost-requestfloor (#300), v20 the blind transfer telemetry (#302), v21 the ask detection
