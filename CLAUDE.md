...
**AND THE SEQUENCE HAS A HOLE IN IT ON PURPOSE: v13 IS SKIPPED. v15 IS THE
NEWEST** — v14 (the PCP queue choice) then v15 (PCP records), each stacked on
the one before, so v15 contains both.** #293 has held `voice-runtime-v13-new-or-existing-20260912` on
its branch since before the PCP queue choice existed, and two open branches
carrying one version would make two different builds indistinguishable at
`/voice/health` — the single thing this constant exists to prevent. So the
queue choice took v14 rather than colliding on v13. A gap in the sequence is
readable; a collision is not. **#293 still has to merge `main` and re-bump
above whatever `main` then carries** — it branched off v10 and contains
neither v11, v12 nor v14, so landing it at v13 would send `/voice/health`
BACKWARDS past three live markers and read as a failed pull.
