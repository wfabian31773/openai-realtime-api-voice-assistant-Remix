...
**AND THE SEQUENCE HAS A HOLE IN IT ON PURPOSE: v13 IS SKIPPED. v64 IS THE
NEWEST, v62 IS WHAT `main` CARRIES, v63 IS A SIBLING CLAIMED BY THE OPEN #326,
AND v57 AND v60 ARE BOTH RETIRED
RATHER THAN REUSED — see THE SURGEON CLAIM below the table for v57.** This
branch held **v60** until 2026-09-23, when it merged the `main` that #324 had
just moved to **v61**; landing at v60 would have sent `/voice/health` BACKWARDS
past a live marker, which reads as a failed pull rather than as a new build, so
it re-bumped to the next free number. **A build must never read v60: it
describes this change and it was never on `main`.** **NEWEST IS NOT THE SAME AS
CONTAINED**, and the two agree here only because the merge made them agree —
v62 contains v61 because `main` was merged in, not because 62 > 61. **v59 IS
STILL CLAIMED BY AN OPEN BRANCH**, #293 (RULE ZERO 2a's gate), which branched
off v10 and contains none of this; it is a SIBLING below, it must merge `main`
and re-bump above v62 before it lands, and its number says nothing about what
it holds. The next ship after this takes **v63**. v19-v24 were siblings off v18 on 2026-09-15 — v19 the PCP lost-requestfloor (#300), v20 the blind transfer telemetry (#302), v21 the ask detection
...
**v61 (#324) IS WHAT `main` CARRIES (merged 2026-09-23, `7de4dd7`), AND v60 IS
RETIRED WITH v57 RATHER THAN REUSED.** v60 was this branch's own number while
it sat below `main`; the moment it merged that `main` it took **v62**, because
two branches carrying one version make two builds indistinguishable at
`/voice/health` — the single thing this constant exists to prevent — and a
number BELOW `main` reads as a failed pull. **v59 is still claimed by the open
#293** and stays reserved for it. Containment is stated once, in the sequence
sentence above; see the v56 row for what v61 measures.
