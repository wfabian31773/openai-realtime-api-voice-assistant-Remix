/**
 * Comprehensive Backfill Script - PRODUCTION
 * Directly connects to Supabase production database using .env credentials
 * 
 * Run: npx tsx scripts/comprehensive-backfill-prod.ts
 */
...
if (!SUPABASE_URL) {
  ...
  console.error('This script requires production database credentials in .env');
  console.error('');
  console.error('Expected .env entries:');
  console.error('  SUPABASE_URL=postgresql://...');
  console.error('  TWILIO_ACCOUNT_SID=...');
  console.error('  TWILIO_AUTH_TOKEN=...');
  process.exit(1);
}

console.log('[INIT] Connecting to Supabase production database...');
...
async function runComprehensiveBackfill() {
  ...
  console.log('COMPREHENSIVE CALL DATA BACKFILL - PRODUCTION');
  console.log('='.repeat(70));
  ...
  console.log('[STEP 2] Fetching Twilio data for calls...');
