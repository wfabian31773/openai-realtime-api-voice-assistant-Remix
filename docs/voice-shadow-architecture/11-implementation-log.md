...
| 2026-08-03 13:15 | **First live findings + fixes** | 9 operator test calls. Director fired 8× across 4 calls, all `inject` — the model complied on every first nudge, escalation never needed. Fixed: (1) record-sourced name disclosure — 3 leaks in one hour on 2 agents, one naming the WRONG person to an outside pharmacy rep; new `record_name_disclosed` rule starting at `author`. (2) Two director false positives — a single legitimate read-back, and `"Fabian, March 17, 1973"` parsing as a name. (3) azul's bundled opener — the pre-context prompt literally instructed it, contradicting the one-question rule 286 lines above. (4) Grade-regression tickets out of the patient queue (VA-47707 fired off n=1). Director extended to no-ivr + after-hours. |
