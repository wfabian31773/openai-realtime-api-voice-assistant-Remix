...
export default function ObservatoryPage() {
  ...
  const [detailAgent, setDetailAgent] = useState<{ id: string; name: string } | 'sage' | null>(null)
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
              ...
              <Card key={s.agentId} className={`border ${toneClasses[d.tone]}`}>
                ...
                <CardContent className="space-y-2 text-sm">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDetailAgent(
                        detailAgent && detailAgent !== 'sage' && detailAgent.id === s.agentId
                          ? null
                          : { id: s.agentId, name: s.agentName },
                      )
                    }
                  >
                    {detailAgent && detailAgent !== 'sage' && detailAgent.id === s.agentId
                      ? 'Hide nuts & bolts ▴'
                      : 'Nuts & bolts — failing checks + worst calls ▾'}
                  </button>
                </CardContent>
              </Card>
            )
              ...
              return (
                <Card className={`border ${toneClasses[d.tone]}`}>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <button
                      className="mt-1 text-xs font-medium text-primary hover:underline"
                      onClick={() => setDetailAgent(detailAgent === 'sage' ? null : 'sage')}
                    >
                      {detailAgent === 'sage'
                        ? 'Hide nuts & bolts ▴'
                        : 'Nuts & bolts — guards, director, tools, telemetry ▾'}
                    </button>
                  </CardContent>
                </Card>
              )
        ...
        {detailAgent === 'sage' && <SageDrilldown days={windowDays} />}
        {detailAgent && detailAgent !== 'sage' && (
          <OpsHubDrilldown agentId={detailAgent.id} agentName={detailAgent.name} days={windowDays} />
        )}
      </section>
...
// ─── Drill-downs: the nuts and bolts behind the tiles ────────────────────

function OpsHubDrilldown({ agentId, agentName, days }: { agentId: string; agentName: string; days: number }) {
  ...
  if (detail.isLoading) return <p className="mt-4 text-sm text-muted-foreground">Loading nuts &amp; bolts…</p>
  if (detail.isError)
    return (
  ...
  return (
    <div className="mt-4 space-y-4 rounded-lg border bg-muted/20 p-4">
      <h3 className="font-semibold">
        {agentName} — nuts &amp; bolts ({d.windowDays}d)
      </h3>

      <div>
        <h4 className="mb-1.5 text-sm font-medium">Grader checks — which are failing, and why</h4>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {d.graderChecks.map((c) => {
                ...
                return (
                  <tr key={c.grader} className={`border-b last:border-0 ${c.criticalFails > 0 && rate >= 0.1 ? 'bg-red-500/5' : ''}`}>
                    <td className="px-2 py-1.5 font-mono">{c.grader}</td>
                    <td className="px-2 py-1.5 text-right">
                      {c.fails}/{c.total}
                    </td>
                    <td className={`px-2 py-1.5 text-right ${rate >= 0.2 ? 'font-semibold text-red-600 dark:text-red-400' : ''}`}>
                      {pct(rate, 1)}
                    </td>
      ...
      <div>
        <h4 className="mb-1.5 text-sm font-medium">
          Worst calls — every critical failure, with what failed and the transcript one click away
        </h4>
        {!d.worstCalls.length && <p className="text-xs text-muted-foreground">No critical-failure calls in this window.</p>}
...
function SageTranscriptModal({ callLogId, onClose }: { callLogId: string; onClose: () => void }) {
  const t = useQuery<{ id: string; createdAt: string; direction: string | null; outcome: string | null; transcript: string | null }>({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        ...
        {t.isError && (
          <p className="text-sm text-red-600">{(t.error as any)?.response?.data?.error ?? String(t.error)}</p>
        )}
        {t.data && (
...
function SageDrilldown({ days }: { days: number }) {
  ...
  const detail = useQuery<SageDetail>({
  ...
  const d = detail.data!
  return (
    <div className="mt-4 space-y-5 rounded-lg border bg-muted/20 p-4">
      <h3 className="font-semibold">SAGE — nuts &amp; bolts ({d.windowDays}d)</h3>

      {/* Guards */}
      <div>
        <h4 className="mb-1.5 text-sm font-medium">
          Hallucination guards — every incident, with the guard that fired and the transcript
        </h4>
        ...
        <div className="space-y-1.5">
          {d.hallucinations.map((h) => (
            <div key={h.id} className="rounded-md border bg-background p-2 text-xs">
              ...
              {h.slotsOfferedSummary && <p className="mt-1 text-muted-foreground">Slots offered: {h.slotsOfferedSummary}</p>}
              {openIncident === h.id && (
      ...
      {/* Director feed */}
      <div>
        <h4 className="mb-1.5 text-sm font-medium">Director handoff feed — why the director pulled calls</h4>
        <div className="overflow-x-auto rounded-md border bg-background">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b bg-muted/50 text-left uppercase text-muted-foreground">
      ...
      {/* Tool errors */}
      <div>
        <h4 className="mb-1.5 text-sm font-medium">Tool calls — error rate per tool, with the actual error messages</h4>
        <div className="overflow-x-auto rounded-md border bg-background">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b bg-muted/50 text-left uppercase text-muted-foreground">
              ...
              {d.toolErrors.map((t) => {
                ...
                return (
                  <tr key={t.toolName} className={`border-b last:border-0 ${rate >= 0.15 ? 'bg-red-500/5' : ''}`}>
                    ...
                    <td className={`px-2 py-1.5 text-right ${rate >= 0.15 ? 'font-semibold text-red-600 dark:text-red-400' : ''}`}>
                      {pct(rate, 1)}
                    </td>
            ...
            <tbody>
              {d.worstTelemetry.map((t, i) => (
                <tr key={i} className="border-b last:border-0">
                  ...
                  <td className={`px-2 py-1.5 text-right ${(t.maxLatencyMs ?? 0) > 6000 ? 'font-semibold text-red-600 dark:text-red-400' : ''}`}>
                    {t.maxLatencyMs != null ? `${t.maxLatencyMs}ms` : '—'}
                  </td>
                  <td className="px-2 py-1.5 text-right">{t.greetingLatencyMs != null ? `${t.greetingLatencyMs}ms` : '—'}</td>
