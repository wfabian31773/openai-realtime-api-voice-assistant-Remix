...
| 1 · **carry forward into every tool and ticket** | **NO — the largest open gap.** The gates still refuse on `date_of_birth`, `location` and `surgeon` for callers whose record holds all three. **v25 records why inherit failed** (`outcome.carry` on a date-of-birth refusal: `fired` / `no_entry` / `entry_without_dob` / `name_mismatch` / `bad_call_sid`). It does not close the gap. Diagnosis: PR #307. |
