...
| 1 · **carry forward into every tool and ticket** | **PARTIAL.** Date of birth inherit on name match + person-base wipe stopped (v26). Filing tools put chart birth year/month/day on the create payload when the ticket name matches the stored name, even if the model omitted `date_of_birth`. Does not invent a date. Does not require DOB on create-ticket. v27 stopped asking a recognised caller for a surname or a date we already hold; **v28 (draft #310) closed the leftover that undid that** — `### How a call runs` still said `identity_is_certain` false means "more than one person" and told the model to collect last name and date of birth. After #292 that flag is also a unique `patients_master` phone hit. Not deployed — live Replit is still v24. `location` and `surgeon` gates still refuse on a record that holds them. The v25 `carry` enum rides along (`fired` / `no_entry` / `entry_without_dob` / `name_mismatch` / `bad_call_sid`). Diagnosis: PR #307 / `the-dob-carry.md`. |
...
| 2b · DOB in month/day/year parts | **YES, all four lanes** — one copy in `recognisedCallerBlock.ts:162` (`identityAskScript`), composed by optical, surgery, tech and records, plus no-ivr and answering-service. |
