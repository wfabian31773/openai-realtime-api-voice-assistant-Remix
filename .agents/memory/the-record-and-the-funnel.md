...
| 1 · **carry forward into every tool and ticket** | **PARTIAL.** Date of birth inherit on name match + person-base wipe stopped (v26). Filing tools put chart birth year/month/day on the create payload when the ticket name matches the stored name, even if the model omitted `date_of_birth`. Does not invent a date. Does not require DOB on create-ticket. `location` and `surgeon` gates still refuse on a record that holds them. The v25 `carry` enum rides along (`fired` / `no_entry` / `entry_without_dob` / `name_mismatch` / `bad_call_sid`). Diagnosis: PR #307 / `the-dob-carry.md`. |
