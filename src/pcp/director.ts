...
export class PcpDirector {
  ...
  next(callId: string): PcpDirectorDecision {
    ...
    // Peer-to-peer is time-sensitive: minimum professional identity is enough
    // to offer the transfer. Patient context may be collected for the record,
    // but must not delay the live connection.
    if (purpose?.patientContextRequired && state.callPurpose !== 'peer_to_peer') required.push(...PATIENT_FIELDS);
